...
\begin{frame}{Rotation matrix }
	\begin{itemize}
		\item The columns of $\matriceR_B^I$ are orthogonal to each other: $\vectorX_B^\top\vectorY_B=0$, $\vectorY_B^\top\vectorZ_B=0$, $\vectorX_B^\top\vectorZ_B=0$
		\item The columns of $\matriceR_B^I$ are normal: $\vectorX_B^\top\vectorX_B=1$, $\vectorY_B^\top\vectorY_B=1$, $\vectorZ_B^\top\vectorZ_B=1$
		\item$\matriceR_I^B=\left(\matriceR_B^I\right)^{-1}=\left(\matriceR_B^I\right)^\top$
		\item $\rm{SO}(3)$ denotes the group of $3\times3$ orthogonal matrices $\matriceR$ such that $\matriceR^\top = \matriceR$ and $\det(\matriceR)=1$
...
\begin{frame}{Rotation matrix }
	\begin{itemize}
		...
		\end{align*}
		with $\vectorOmega_B^B,\vectorOmega_B^I\inR^3$ are the body and frame $\setR_I$ expressed angular velocity, respectively, and $\left[\star\times\right]\inR^{3\times 3}$ is the skew-matrix of the vector $\star$
	%	\item Orientation can also be minimally represented by: Euler angles, Angle-axis, Quaternions
	\end{itemize}
\end{frame}
\begin{frame}{Orientation representation}
	\begin{itemize}
		\item Orientation can also be minimally represented using: Euler angles, Angle-axis, Quaternions
		\item Transformations exist to move (bidirectionnally) from the rotation matrix  to any other minimal orientation representation
		\item  Example: Euler angles $\rightarrow$ Rotation matrix
		\item Example: Rotation matrix $\rightarrow$ Euler angles
	\end{itemize}
\end{frame}
...
\begin{frame}{Homegeneous transformation}
	...
	\begin{align*}
		\left(\matriceT_B^I\right)^{-1} = \matriceT^B_I &= \begin{bmatrix}
			\left(\matriceR_B^I\right)^\top & -\left(\matriceR_B^I\right)^\top \vectorO_B\\
...
In the case of articulated robot, the position and orientation of the end-effector frame  $\setR_{ee}$ w.r.t to $\setR_I$ can be obtained through the homogeneous transformation matrix composition as a function of $\vectorQ$
	...
	%	$\matriceT_{ee}^I$ is denoted as the \textbf{forward kinematic model}
		\begin{figure}
			...
			\begin{tikzpicture}[auto, node distance=1.3cm, >=latex]
				...
				% Nodes
				\node[block] (b1) {Forward kinematic model};
				
				% Inputs
				\draw[->] ($(b1.west)+(-1,0)$) -- (b1.west) node[midway, above] {$\vectorQ$};
				
				% Outputs
				\draw[->] (b1.east) -- ($(b1.east)+(1,0)$)node[midway, above] {$\matriceT_{ee}^I$};
...
\end{frame}
\begin{frame}{Denavit-Hartenberg method}
	For each of the previous examples, $\matriceT_j^{j-1}$ is computed for $j=1,2,3$. Then, the forward kinematics model is obtained by computing 
	\begin{equation*}
...
%	\setbeamerfont{enumerate item}{size=\Large}
	\begin{figure}
		...
		\begin{tikzpicture}[auto, node distance=1.3cm, >=latex]
			...
			% Nodes
			\node[block] (b1) {Inverse kinematics model};
...
\begin{frame}{Inverse kinematics - Analytical approach}
	...
	\begin{minipage}{.4\columnwidth}
		1st step: find the forward kinematics model
%		\begin{equation*}
...
\begin{frame}{Inverse kinematics - Analytical approach}
	\begin{figure}
		\centering
		\includegraphics[width=0.4\columnwidth]{2DOF-Sol1}
		\includegraphics[width=0.4\columnwidth]{2DOF-Sol2}
		\caption{Two possible inverse kinematics solutions}
	\end{figure}
	The choice of solution is based on the desired end-effector orientation $\theta_d=q_1+q_2$
\end{frame}
\begin{frame}{Inverse kinematics - Analytical approach}
	\begin{itemize}
	\item In general, the forward kinematics model is 
	\begin{itemize}
		\item nonlinear w.r.t $\vectorQ$
		\item strictly depending on robot structure, DoF type and number
	\end{itemize}
	$\Rightarrow$ No systematic method to compute analytically $\vectorQ$ for all the robots
...
\begin{itemize}
	\item Let us assume $\vectorX_d$ is the desired end-effector pose, with $\vectorX_d=f(\vectorQ_d)$
	\item The inverse kinematics problem consists of finding the root of the algebraic equation 
	\begin{equation*}
		\vectorX_d-f(\vectorQ_d) = 0
	\end{equation*}
	\item \textbf{Method:} use numerical solvers to find the root $\vectorQ_d$ solution of the inverse kinematics problem
\end{itemize}
\end{frame}
\begin{frame}{Inverse kinematics - Numerical approach}
	Newton-Raphson method:
	\begin{itemize}
		\item Start with inital state $\vectorQ_i$
		\item Perform Taylor expansion around $\vectorQ_i$:
		\begin{align*}
			\vectorX_d = f(\vectorQ_d)&=f(\vectorQ_i+\Delta\vectorQ_i),\ \Delta\vectorQ_i = \vectorQ_d-\vectorQ_i\\
			&=f(\vectorQ_i) + \dfrac{\partial f}{\partial\vectorQ}\Delta\vectorQ_i+ {\rm H.o.T}\\
			\vectorX_d	&\approx f(\vectorQ_i) + \dfrac{\partial f}{\partial\vectorQ}\Delta\vectorQ_i\\
			\Rightarrow \Delta\vectorX_d &=\dfrac{\partial f}{\partial\vectorQ}\Delta\vectorQ, \ \Delta\vectorX_d = \vectorX_d-\vectorX 
		\end{align*}
	\end{itemize} 
\end{frame}
	\begin{frame}{Inverse kinematics - Numerical approach}
		Algorithm:
		\begin{enumerate} 
			\item Starting with inital state $\vectorQ_i$, compute a solution $\vectorQ_{d_i}$
			\begin{equation*}
				\vectorQ_{d_i} = \vectorQ_{i}+\left[\dfrac{\partial f}{\partial\vectorQ}\right]^{-1}\Delta\vectorX_d, \ \Delta\vectorX_d = \vectorX_d-\vectorX 
			\end{equation*}
			\item Recompute a new iteration $\vectorQ_{d_{i+1}}$ with initial guess $\vectorQ_i =\vectorQ_{d_i}$ until convergence
		\end{enumerate}
\end{frame}
\begin{frame}{Inverse kinematics - Numerical approach}
	\begin{figure}
		\centering
		\includegraphics[width=0.85\textheight]{Newton-raphson}
		\caption{Iterative computation for numerical inverse kinematics with Newton-Raphson method.}
	\end{figure}
\end{frame}
\begin{frame}{Inverse kinematics - Numerical approach}
%	\begin{itemize}
		%\item 
		In general, dedicated numerical inverse-kinematic approaches are more elaborated to account for
		\begin{itemize}
			\item Joints position limits
			\item Avoid self-collision
			\item Avoid bad numerical behavior especially when the desired pose $\vectorX_d$ is out of reach (outside the workspace)
			\item Optimize the solutions (w.r.t some criteria) in case of the robot redundancy, e.g., ensure solutions continuity when $\vectorX_d$ is time-varying
		%\end{itemize} 
	\end{itemize}
\end{frame}