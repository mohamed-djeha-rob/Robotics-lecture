    ...
    \begin{columns}
        \begin{column}{0.48\textwidth}
            \begin{block}{Timing Belts \& Pulleys}
                \begin{center}
                    \includegraphics[scale=0.05]{./robotique_figures/pulley_belt_transmission.jpg} 
                \end{center}
                \begin{itemize}
                    \footnotesize
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Cable Drives}
                \begin{center}
                    \includegraphics[scale=0.1]{./robotique_figures/cable_driven_transmission.png} 
                \end{center}
                \begin{itemize}
                    \footnotesize
...
\begin{frame}{Summary: Complete Robot System}
    \begin{center}
        \begin{tikzpicture}[scale=0.8]
            ...
            \node[draw, rectangle, fill=blue!20, minimum width=2cm] (controller) {Controller};
            \node[draw, rectangle, fill=green!20, right=of controller, minimum width=2cm] (amplifier) {Amplifier};
            \node[draw, rectangle, fill=red!20, right=of amplifier, minimum width=2cm] (motor) {Motor};
            \node[draw, rectangle, fill=orange!20, right=of motor, minimum width=2cm] (gearbox) {Gearbox};
            \node[draw, rectangle, fill=purple!20, right=of gearbox, minimum width=2cm] (joint) {Joint};
            \node[draw, rectangle, fill=brown!20, right=of joint, minimum width=2cm] (ee) {End Effector};
            
            % Sensors
            \node[draw, rectangle, fill=yellow!20, below=of motor, minimum width=2cm] (encoder) {Encoder};
            \node[draw, rectangle, fill=yellow!20, below=of ee, minimum width=2cm] (ftsensor) {F/T Sensor};
            
            % Connections
            \draw[->, thick] (controller) -- node[above] {Command} (amplifier);
            \draw[->, thick] (amplifier) -- node[above] {Current} (motor);
            \draw[->, thick] (motor) -- node[above] {High speed} (gearbox);
            \draw[->, thick] (gearbox) -- node[above] {Low speed} (joint);
            \draw[->, thick] (joint) -- node[above] {Motion} (ee);
            
            \draw[->, thick] (encoder) -- node[left] {Position} (motor);
            \draw[->, thick] (encoder) -| (controller);
            \draw[->, thick] (ftsensor) -- node[left] {Force} (ee);
            \draw[->, thick] (ftsensor) -| (controller);
            
            \draw[<-, thick] (controller.west) -- ++(-1,0) node[left] {Trajectory Plan};
            \draw[->, thick] (ee.east) -- ++(1,0) node[right] {Task Execution};
        \end{tikzpicture}
    \end{center}
